...
def _page_brief(status: dict, now_et: str) -> str:
    ...
    # Signal Map — overlay brief conviction scores to match latest_updates panel.
    # signal_scores.json is updated every A1 cycle (~15 min) with raw algorithmic
    # scores. The brief's high_conviction_longs and watch_list are generated by
    # Claude's qualitative analysis (catalysts, earnings, insider activity) and
    # match what appears in the Latest Updates panel. Override raw scores with
    # brief scores for any symbol that appears in those sections.
    _scored = dict(_signal_map_data())
    for _section in ("high_conviction_longs", "watch_list"):
        for _item in brief.get(_section, []):
            _sym = _item.get("symbol")
            _score = _item.get("score")
            if _sym and _score is not None:
                _existing = dict(_scored.get(_sym, {}))
                _existing["score"] = _score
                _scored[_sym] = _existing
    signal_map_html = ""
    if _scored:
        _sm_tiles = ""
        ...
        signal_map_html = (
            ...
            f'color:var(--bbb-fg-dim);margin-left:8px">{_n} symbols</span>'
            f'<span style="font-size:11px;font-weight:400;letter-spacing:0;text-transform:none;'
            f'color:var(--bbb-fg-dim);margin-left:8px">· brief {gen_display}</span></div>'
