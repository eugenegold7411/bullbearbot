...
def _signal_map_data() -> dict:
    """Merge signal_scores.json base with morning_brief_full.json overlay.

    Returns {sym: {score, direction, conviction, tier, catalyst_display,
                   earnings_days_away, iv_rank, brief_section}} dict.
    The brief overlay contributes Claude's qualitative catalyst text, IV rank
    parsed from a2_strategy_note, and the section classification (LONG/BEAR/WATCH/AVOID).
    Brief scores override raw algorithmic scores when available.
    """
    import re as _re
    try:
        ...
        if not p.exists():
            return {}
        scored = json.loads(p.read_text()).get("scored_symbols", {})
        if not scored:
            return {}
    except Exception:
        return {}
    ...
    try:
        _brief = json.loads(
            (BOT_DIR / "data/market/morning_brief_full.json").read_text()
        )
        for _sk, _lbl in [
            ("high_conviction_longs",   "LONG"),
            ("high_conviction_bearish", "BEAR"),
            ("watch_list",              "WATCH"),
            ("avoid_list",              "AVOID"),
        ]:
            for _e in _brief.get(_sk, []):
                _s = _e.get("symbol", "")
                if not _s or _s in overlay:
                    continue
                _iv = None
                _m = _re.search(r"iv_rank=(\d+)", _e.get("a2_strategy_note", ""))
                if _m:
                    _iv = int(_m.group(1))
                overlay[_s] = {
                    "brief_section":  _lbl,
                    "catalyst_brief": _e.get("catalyst", ""),
                    "score_brief":    _e.get("score"),
                    "iv_rank_brief":  _iv,
                }
    except Exception:
        pass
    ...
    result = {}
    for sym, data in scored.items():
        ov  = overlay.get(sym, {})
        _iv = ov.get("iv_rank_brief")
        if _iv is None:
            _iv = _iv_rank_local(sym)
        _sc = (ov["score_brief"] if ov.get("score_brief") is not None
               else data.get("score", 0))
        result[sym] = {
            "score":              _sc,
            "direction":          data.get("direction", "neutral"),
            "conviction":         data.get("conviction", "low"),
            "tier":               data.get("tier", ""),
            "catalyst_display":   ov.get("catalyst_brief") or data.get("primary_catalyst", ""),
            "earnings_days_away": data.get("earnings_days_away"),
            "iv_rank":            _iv,
            "brief_section":      ov.get("brief_section", ""),
        }
    return result
...
def _page_brief(status: dict, now_et: str) -> str:
    ...
    # Universe grid — full sortable table replacing the compact tile view.
    # Data from _signal_map_data() which merges signal_scores.json (all symbols)
    # with morning_brief_full.json overlay (richer catalyst text + IV rank).
    ...
    if _scored:
        ...
        _rows = ""
        for _sym, _d in sorted(_scored.items(), key=lambda x: -(x[1].get("score") or 0)):
            _sc    = _d.get("score") or 0
            _dir   = (_d.get("direction") or "neutral").lower()
            _conv  = (_d.get("conviction") or "low").upper()
            _tier  = _d.get("tier") or ""
            _cat   = (_d.get("catalyst_display") or "")[:60]
            _eda   = _d.get("earnings_days_away")
            _iv    = _d.get("iv_rank")
            _sect  = _d.get("brief_section") or ""
            _sc_c  = ("var(--bbb-profit)" if _sc >= 70
                      else "var(--bbb-warn)" if _sc >= 50
                      else "var(--bbb-fg-dim)")
            if _dir == "bullish":
                _arr = "&#x2191;"; _arr_c = "var(--bbb-profit)"; _dir_sv = "2"
            elif _dir == "bearish":
                _arr = "&#x2193;"; _arr_c = "var(--bbb-loss)"; _dir_sv = "0"
            else:
                _arr = "&#x2192;"; _arr_c = "var(--bbb-fg-dim)"; _dir_sv = "1"
            _conv_c  = ("var(--bbb-profit)" if _conv == "HIGH"
                        else "var(--bbb-warn)" if _conv == "MEDIUM"
                        else "var(--bbb-fg-dim)")
            _conv_sv = "2" if _conv == "HIGH" else "1" if _conv == "MEDIUM" else "0"
            _tier_sv = "2" if _tier == "core" else "1" if _tier == "growth" else "0"
            _sect_sfx = (
                f'<span style="font-size:10px;color:{_SECT_C.get(_sect,"")};'
                f'margin-left:5px;opacity:0.75">{_sect}</span>'
            ) if _sect else ""
            _eda_sv  = str(_eda) if _eda is not None else "9999"
            _eda_txt = str(_eda) if _eda is not None else "—"
            _iv_sv   = f"{_iv:.0f}" if _iv is not None else "-1"
            _iv_txt  = f"{_iv:.0f}" if _iv is not None else "—"
            _rows += (
                f'<tr>'
                f'<td style="text-align:left">'
                f'<b style="{S_MONO};font-size:12px;color:var(--bbb-fg)">{_sym}</b>'
                f'{_sect_sfx}</td>'
                f'<td style="text-align:right;{S_MONO};font-size:13px;color:{_sc_c};'
                f'font-weight:500" data-v="{_sc:.1f}">{_sc:.0f}</td>'
                f'<td style="text-align:center;color:{_arr_c}" data-v="{_dir_sv}">{_arr}</td>'
                f'<td style="text-align:left;{S_MONO};font-size:11px;color:{_conv_c}"'
                f' data-v="{_conv_sv}">{_conv}</td>'
                f'<td style="text-align:left;font-size:11px;color:var(--bbb-fg-muted)"'
                f' data-v="{_tier_sv}">{_tier}</td>'
                f'<td style="text-align:left;font-size:11px;color:var(--bbb-fg-muted)">{_cat}</td>'
                f'<td style="text-align:right;{S_MONO};font-size:12px;color:var(--bbb-fg-muted)"'
                f' data-v="{_eda_sv}">{_eda_txt}</td>'
                f'<td style="text-align:right;{S_MONO};font-size:12px;color:var(--bbb-fg-muted)"'
                f' data-v="{_iv_sv}">{_iv_txt}</td>'
                f'</tr>'
            )
        _n  = len(_scored)
        _js = (
            '<script>(function(){'
            'var t=document.getElementById("uni-tbl"),'
            'tb=t.querySelector("tbody"),'
            'hs=t.querySelectorAll("th"),'
            'sc=1,sa=false;'
            'function ds(c){'
            'if(c===sc){sa=!sa;}else{sc=c;sa=(c===0||c===5||c===6||c===7);}'
            'var rs=Array.from(tb.rows);'
            'rs.sort(function(a,b){'
            'var av=a.cells[c].getAttribute("data-v")||a.cells[c].textContent.trim(),'
            'bv=b.cells[c].getAttribute("data-v")||b.cells[c].textContent.trim(),'
            ...
            'hs.forEach(function(h,i){h.style.opacity=i===sc?"1":"0.6";});'
            '}'
            'hs.forEach(function(h,i){'
            'h.style.cursor="pointer";'
            'h.title="Click to sort";'
            'h.onclick=function(){ds(i);};'
            '});'
            '})();</script>'
        )
        signal_map_html = (
            f'<div style="{S_CARD}">'
            f'<div style="{S_LBL};color:var(--bbb-fg-muted)">Universe'
            f'<span style="font-size:11px;font-weight:400;letter-spacing:0;text-transform:none;'
            f'color:var(--bbb-fg-dim);margin-left:8px">{_n} symbols · brief {gen_display}</span></div>'
            f'<div class="table-wrap" style="max-height:380px;overflow-y:auto">'
            f'<table class="data-table" id="uni-tbl">'
            ...
            f'<th style="text-align:left">Conv</th>'
            f'<th style="text-align:left">Tier</th>'
            f'<th style="text-align:left">Catalyst</th>'
            f'<th style="text-align:right" title="Earnings days away">EDA</th>'
            f'<th style="text-align:right" title="IV rank (0-100)">IV</th>'
            f'</tr></thead>'
            f'<tbody>{_rows}</tbody>'
            f'</table></div>'
            + _js
